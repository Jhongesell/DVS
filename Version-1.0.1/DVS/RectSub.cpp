...
int RectSub::addProjNs(ldouble **A, int *map, ldouble fac)
{
   int i, j, k, eq1 = 0, eq2 = 0, b = 0;
   for (i = 0; i < np; i++)
   {
...
int RectSub::addProjNs(MatrizDispersa *A, int *map, ldouble fac)
{
   int i, j, k, eq1 = 0, eq2 = 0, b = 0;
   for (i = 0; i < np; i++)
   {
...
// Se asume coeficientes constantes
   ...
   if(ExpX == 1.0  || Cb[0] == 0.0) val = Ca[0] / (h[0] * h[0]);
   else val = ( (PeX / 2.0) * (ExpX + 1.0) / (ExpX - 1.0) ) / (h[0] * h[0]); //ui,j,k

   if (nDim > 1)
   {
      if(ExpY == 1.0 || Cb[1] == 0.0) val += Ca[1] / (h[1] * h[1]); //ui,j,k
      else val += ( (PeY / 2.0) * (ExpY + 1.0) / (ExpY - 1.0) ) / (h[1] * h[1]); //ui,j,k
   }
   if (nDim > 2)
   {
      if(ExpZ == 1.0 || Cb[2] == 0.0) val += Ca[2] / (h[2] * h[2]); //ui,j,k
      else val += ( (PeZ / 2.0) * (ExpZ + 1.0) / (ExpZ - 1.0) ) / (h[2] * h[2]); //ui,j,k
   }
   coef[0] = Cc + 2.0 * val;//coeficientes Uij
   if(ExpX == 1.0 || Cb[0] == 0.0)
   {
      coef[1] = -(1.0 / h[0]) * ( Ca[0] / h[0]); //coeficientes ui,j+1,k
      coef[2] = -(1.0 / h[0]) * ( Ca[0] / h[0]); // Coeficientes ui,j-1, k
   }
   else
   {
      coef[1] = (1.0 / h[0]) * ( Cb[0] / 2.0 - ( (PeX / 2.0) * (ExpX + 1.0) / (ExpX - 1.0) ) / h[0]); //coeficientes ui+1,j,k
      coef[2] = -(1.0 / h[0]) * ( Cb[0] / 2.0 + ( (PeX / 2.0) * (ExpX + 1.0) / (ExpX - 1.0) ) / h[0]); // Coeficientes ui-1,j,k
   }
   if (nDim > 1)
   {
      if(ExpY == 1.0 || Cb[1] == 0.0)
      {
         coef[3] = -(1.0 / h[1]) * ( Ca[1] / h[1]); //coeficientes ui,j+1,k
         coef[4] = -(1.0 / h[1]) * ( Ca[1] / h[1]); // Coeficientes ui,j-1, k
      }
      else
      {
         coef[3] = (1.0 / h[1]) * ( Cb[1] / 2.0 - ( (PeY / 2.0) * (ExpY + 1.0) / (ExpY - 1.0) ) / h[1]); //coeficientes ui+1,j,k
         coef[4] = -(1.0 / h[1]) * ( Cb[1] / 2.0 + ( (PeY / 2.0) * (ExpY + 1.0) / (ExpY - 1.0) ) / h[1]); // Coeficientes ui-1,j, k
      }
   }
   if (nDim > 2)
   {
      if(ExpZ == 1.0 || Cb[2] == 0.0)
      {
         coef[5] = -(1.0 / h[2]) * (Ca[2] / h[2]); //coeficientes ui,j+1,k
         coef[6] = -(1.0 / h[2]) * (Ca[2] / h[2]); // Coeficientes ui,j-1, k
      }
      else
      {
         coef[5] = (1.0 / h[2]) * (Cb[2] / 2.0 - ( (PeZ / 2.0) * (ExpZ + 1.0 ) / (ExpZ - 1.0) ) / h[2]); // coeficientes ui,j,k+1
         coef[6] = -(1.0 / h[2]) * (Cb[2] / 2.0 + ( (PeZ / 2.0) * (ExpZ + 1.0 ) / (ExpZ - 1.0) ) / h[2]); // coeficientes ui,j,k-1
      }
   }
...
Solvable *RectSub::genInverse(int *map, ldouble fac)
{
   ...
   if (fac != 0.0) b2 = addProjNs(A, map, fac);
   ...
   if (bsym)
   {
      // Resolucion del sistema lineal por medio del metodo Cholesky
      ss = new (nothrow) BandCholesky(n, A);
      if (ss == NULL) ce.memoryError("ss in BandCholesky");
   }
   else
   {
